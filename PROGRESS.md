...
- **Milestone:** **▶ M4 (v1) IN PROGRESS — MVP gate satisfied, proceeding on the autonomous loop.**
  ...
  **DECISIONS RECORDED this unit:**
  ...
  (72) ✅ **this firing** — **`doctor.Collect` — the detector-battery composition seam** (guard
  `DOCTOR-BATTERY-COMPLETE`; `internal/doctor/collect.go` + `collect_test.go`), the unit that turns the five
  standalone detectors (#1–#5) into ONE fixed battery (DESIGN §7.5). Until now the doctor package was imported
  by zero non-test files — five verdicts existed but nothing composed or exposed them; this is the single
  registration point the (next) `wi doctor`/`check` command calls so the command never learns the detector list.
  Two new symbols: `Observations{Orphans []gc.Candidate; Drift []DriftObservation; Mirrors []mirror.Snapshot;
  PendingOps []journal.OpRecovery; ParkedLands []landstate.TaskLand}` — one field per detector, each the exact
  slice that detector consumes — and `Collect(obs Observations) []Finding`, which runs the FULL battery and
  returns the union of their Findings (which `WorstExit` then reduces to the run's exit code). **Like every
  detector it is PURE:** no IO, no network, no clock — ALL gathering (`gc.Inspect`, `mirror.Load`, `journal.Scan`,
  `landstate.Load`, `isolate.Inspect`) is the command's job, which is exactly what holds DESIGN §2 invariant #3
  (no hidden network) on this path and lets the unit test exercise the whole battery with plain fixtures, no
  workspace. The append order is stable but NOT load-bearing (WorstExit ranks by severity, not position; the
  command projects each finding independently). Adding the remaining §7.5 detectors is one new `Observations`
  field + one `append` here, command untouched. **Test-first:** `TestCollectRunsEveryDetector` builds one
  Observations bundle whose every field carries that detector's own trigger fixture (the markerless `gc.Candidate`,
  the behind-origin `mirror.Snapshot`, the roll-forward `journal.OpRecovery`, the blocked `landstate.TaskLand`,
  the ClassMissingWorktree `DriftObservation`) and asserts the returned Findings' set of `Detector` values
  covers ALL of {orphans, mirror, journal, land, isolate} — so a detector silently dropped from the battery
  (which would make `wi doctor` stop diagnosing that whole trouble class while still exiting 0) reddens. Compile
  RED first (`undefined: doctor.Observations`/`doctor.Collect`). **Registered mutants `DOCTOR-BATTERY-COMPLETE`
  (two-sided):** delete the `DetectMirrorStaleness(obs.Mirrors)` append → "mirror" missing → membership assertion
  RED (`saw map[isolate:… journal:… land:… orphans:…]`); independently delete the `DetectDrift(obs.Drift)` append
  → "isolate" missing → RED (`saw map[journal:… land:… mirror:… orphans:…]`). Both confirmed RED→revert→GREEN
  this firing. Full gate GREEN (29 pkgs ok + `? schema`) + gofmt clean (internal/doctor) + go vet clean. **Unit
  determination:** picked via a 7-agent ultracode Workflow (5 parallel package mappers → synthesize → adversarial
  critic) that APPROVED `doctor.Collect` as the smallest highest-leverage next unit (detectors exist but nothing
  composes them; zero contract edits) over the heavier CLI-seam alternative; the critic's strengthening note (add
  a second mutant) is honored by the two-sided pair above. NEXT M4 unit — the **`wi doctor`/`check` CLI seam**:
  a `cmd_doctor` handler that does the IO to build `Observations`, calls `Collect`, projects the `[]Finding` onto
  the envelope, and takes its exit from `WorstExit` (reconciling doctor's `WorstExit` against the cli pipeline's
  `ExitFor` exit authority — the one cross-cutting decision this seam forces) + a `check` alias + the
  HELP-REGISTRY-SYNC row; this also needs the contract home for WARNING findings (today `Warnings[].Code` is a
  CLOSED enum with only `hydrate_skipped`/`base_behind_ssot`). The remaining detectors (#6 **SSOT cleanliness**,
  **lock inventory + liveness**, **`.wi` parseability**, **environment probes** — the last gated on §7 #7) are
  each one more `Observations` field + `append`, addable before or after the command. Then bounded `--fix`.
  HEAL-8 is the LAST M4 item.

  (71) ✅ **(prior firing)** — **`doctor.DetectDrift` — the three-way isolate-drift detector** (guard
...
| DOCTOR-BATTERY-COMPLETE (M4 HEAL-8) | the detector-battery composition seam `doctor.Collect(Observations) []Finding` + the `Observations` bundle (`internal/doctor/collect.go` + `collect_test.go`) — the single registration point that runs the FULL fixed battery (DESIGN §7.5) so the `wi doctor` command never learns the detector list. PURE: no IO/network/clock (all gathering is the command's job), the union of every detector's Findings, which `WorstExit` reduces to the exit. `TestCollectRunsEveryDetector` feeds one Observations bundle whose every field carries that detector's trigger fixture and asserts the returned `Detector` set covers ALL of {orphans, mirror, journal, land, isolate}. Two-sided mutant: delete the `DetectMirrorStaleness(obs.Mirrors)` append → "mirror" absent from the set → membership assertion RED (`saw map[isolate journal land orphans]`); independently delete the `DetectDrift(obs.Drift)` append → "isolate" absent → RED (`saw map[journal land mirror orphans]`) — either dropped detector reddens, pinning that Collect runs the WHOLE battery, not a subset (a forgotten detector cannot hide behind a clean exit). Compile RED first (`undefined: doctor.Observations`/`doctor.Collect`). Both Darwin RED→revert→GREEN + gofmt clean (internal/doctor) + go vet clean (29 pkgs ok) |
